...
import type { ConvertStage, ConvertStageEvent } from '@codify/contracts'

type ProgressSink = (event: ConvertStageEvent) => void

export interface ConvertProgressReporter {
  report(stage: ConvertStage): void
...
export const CONVERT_PROGRESS_LABELS = {
  ...
  completed: '转换完成',
  failed: '转换失败',
} satisfies Record<ConvertStage, string>

const AGENT_PROGRESS_STAGES = new Set<ConvertStage>(['observe', 'plan', 'rewrite'])

export function createConvertProgressReporter(onProgress?: ProgressSink): ConvertProgressReporter {
  const reportedAgentStages = new Set<ConvertStage>()

  const report = (stage: ConvertStage) => {
    onProgress?.({
      stage,
...
function normalizeAgentStage(rawEvent: string): ConvertStage {
  if (rawEvent === 'apply' || rawEvent === 'optimize') return 'rewrite'
  return rawEvent as ConvertStage
}
