...
import type { ConvertStage } from "@codify/contracts";
import { env } from "../../platform/config/env.ts";
import type { ConvertProgressReporter } from "../shared/convertProgress.ts";
import type {
  AiEnhanceResult,
  CodegenResult,
  FigmaNodeRef,
...
import {
  formatUserError,
  type AiEnhanceStage,
} from "../../platform/errors/userErrorEvents.ts";
...
import { SourceInsightService } from "../../sourceInsight/insight/sourceInsightService.ts";
...
import { convertHtmlCssToInlineStyle } from "./utils/convertHtmlCssToInlineStyle.ts";
...
export class AiEnhanceService {
    ...
    try {
      ...
      const result = await stage.run("agent_visual_repair", () =>
        runVisualRepair({
          ...
          timeout: env.aiEnhance.llmTimeoutMs,
  ...
  private async prepareCurrentHtml(
    ...
    try {
      ...
      const inlinedFragment = convertHtmlCssToInlineStyle(
        htmlFragment,
        input.codegenResult.css,
      ).trim();
      return inlinedFragment;
    } catch (error) {
      this.loggingService.warn(
        "AI enhance inline-style conversion fell back to raw html+css",
        {
          runId,
          module: "aiEnhance",
          source: "backend",
    ...
    return {
      run: async <T>(
        ...
        if (stage === "render_baseline" || stage === "render_current") {
          convertProgress.report(stage satisfies ConvertStage);
        }
        try {
